...
function read_stata(fn::String; chunks::Int=10, keep_original = false)
    ...
    for i in 1:ncol(rdf)
        ...
        # into CategoricalArrays with the value labels
        if typelist[i] in (65528, 65529, 65530) && haskey(lblname_dict, i)
            rdf[!, varlist[i]] = categorical(recode2(rdf[!,varlist[i]], value_labels[lblname_dict[i]], keep_original))
        end
...
function recode2(vv::AbstractVector,dd::Dict, keep_original=false)
    ...
    return [ haskey(dd,vv[i]) ? string(dd[vv[i]]) : string("(",vv[i],")") for i in 1:length(vv)]
end
...
function prepare_df(outdf; verbose=verbose)
    ...
    for i in 1:ncol(outdf)
        if !isa(outddf[:,i], CategoricalArray) && 
            !in(_eltype2(outdf[:,i]), [Bool, Int8, Int16, Int32, Float32, Float64, Date, DateTime, String, String1, String3, String7, String15, String31, String63, String127, String255])
            notallowed[i] = true
        end
    end
